...
function renderPost(job) {
    ...
    // Category/State related links
    var _rl = buildRelatedLinks(job);
    var categoryLinks = '';
    if (_rl.length > 0) {
        categoryLinks = '<div style="margin-top:24px;padding:16px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;"><p style="font-weight:600;margin-bottom:8px;color:#334155;font-size:15px;">Related Categories:</p><div style="display:flex;flex-wrap:wrap;gap:4px 16px;">' + _rl.join('') + '</div></div>';
    }

    // Assemble
    const html = `${breadcrumb}${backBtn}${header}${sectionsHTML}${linksHTML}${shareHTML}${relatedHTML}${categoryLinks}`;
